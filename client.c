...
void send_request(int fd)
{
   ...
   ssize_t num = 0;
	...
	// while input to client != EOF
   while ((num = getline(&line, &size, stdin)) >= 0)
   {
	// write client input to server
      write(fd, line, num);

	// read data in file from server
	// clear buffer so data from previous GET is not printed again
	char buffer[256];
	for (int i = 0; i < 256; i++) {
		buffer[i] = '\0';
	}
	if (read(fd, buffer, 256) < 0) {
		free(line);
		perror("Unable to read from server to client\n");
		exit(1);		
	}
	printf("%s", buffer);
   }
